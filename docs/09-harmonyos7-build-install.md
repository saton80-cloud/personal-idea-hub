...
- 项目内新增子创意、项目进度推进
- 应用私有离线缓存、待同步队列与双向增量同步
- 同记录多端修改时按`updatedAt`自动采用最新版本
- 上午计划和晚间复盘的应用内提醒

系统后台定时通知和验收多分支的完整编辑器不在本次鸿蒙适配范围内。
...
4. 手机会上传本地待处理修改，服务器按`updatedAt`保留较新版本，再按变更序号拉取服务器最终状态。
...
按[验收清单](08-acceptance-checklist.md)完成真机扫码、首次同步、离线新增、恢复网络、最新更新时间合并和界面检查后，再保留签名HAP作为个人安装包。
